...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('book-upms-systemid', systemid);
		$.cookie('book-upms-systemname', systemname);
		$.cookie('book-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('book-upms-systemid') || 1;
	var systemname = $.cookie('book-upms-systemname') || 'book-upms-server';
	var systemtitle = $.cookie('book-upms-systemtitle') || '图书管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
